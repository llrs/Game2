...
class Maping(object):
    ...
    def __init__(self, large = 4):
        ...
        self.middle = large+1
        self.obj_places = np.eye(self.large, dtype = object)
        self.nam_places = np.eye(self.large, dtype = '<U20')
        self.count =  {}
        ...
        options = (Place("sea"),                  #1
                      ...
                      Place("jungle"),               #4
                      Place("cave"),                 #5
                      Place("abandoned house"),      #6
                      Place("forest"),               #7
                      Place("house"),                #8
                      Place("town"),                 #9
                      Place("city"),                 #10
                      Place("countryside"),          #11
                      Place("desert"),               #12
                      Place("castle"),               #13
                      Place("mountain"),             #14
                      Place("high mountain"))        #15
        
        #Check pygames and gamesdev. It doesn't seem to work well
        self.obj_places = np.array([[Place() for i in range(self.large)]
                                    for j in range(self.large)], dtype = object)
        ...
        self.obj_places[self.middle,self.middle].place = "house"
        self.obj_places[self.middle,self.middle+1].place = "cave"
        self.obj_places[self.middle/2,self.middle/2].place = "town"
        self.obj_places[self.middle/2-1,self.middle/2].place = "high mountain"
        self.obj_places[self.middle/2,self.middle/2-1].place = "high mountain"
        self.obj_places[self.middle/2-1,self.middle/2-1].place = "high mountain"
        #Further development place a river from the high mountains in near the town  until the city
        self.obj_places[self.middle/2+1,self.middle/2].place = "river"
        self.obj_places[self.middle*3/2,self.middle*3/2].place = "city"
        self.obj_places[self.middle*3/2+1,self.middle*3/2].place = "mountain"
        self.obj_places[self.middle*3/2,self.middle*3/2+1].place = "mountain"
        self.obj_places[self.middle*3/2+1,self.middle*3/2+1].place = "mountain"
        self.obj_places[self.middle*3/2-1, self.middle*3/2].place = "lake"

        # Get the names for each positions
        # Prepares a list to count how much of each is in the map
        places  =  []
        for i in range(self.large):
            for j in range(self.large):
                self.nam_places[i,j] = self.obj_places[i,j].place
                places.append(self.obj_places[i,j].place)
        for place in set(places):
            self.count[place] = places.count(place)
